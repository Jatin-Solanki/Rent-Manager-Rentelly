...
const previousTenantsCol = collection(db, 'previousTenants');
...
// Firebase handlers for Previous Tenants
export const fetchPreviousTenants = async (): Promise<Tenant[]> => {
  try {
    const currentUser = auth.currentUser;
    if (!currentUser) {
      console.error("No authenticated user for fetching previous tenants");
      throw new Error("No authenticated user");
    }
    
    console.log("Fetching previous tenants for user:", currentUser.uid);
    const q = query(previousTenantsCol, where("ownerId", "==", currentUser.uid));
    const snapshot = await getDocs(q);
    
    if (snapshot.empty) {
      console.log("No previous tenants found for this user");
      return [];
    }
    
    const tenants = snapshot.docs.map(doc => {
      const rawData = doc.data();
      console.log("Raw previous tenant data from Firestore:", { id: doc.id, ...rawData });
      
      const processedData = convertTimestamps(rawData);
      
      // Ensure all required tenant fields are present
      const tenant: Tenant = { 
        id: doc.id, 
        name: processedData.name || 'Unknown',  // Provide default if name is missing
        contactNo: processedData.contactNo || '',
        memberCount: processedData.memberCount || 0,
        rentAmount: processedData.rentAmount || 0,
        roomDetails: processedData.roomDetails || '',
        about: processedData.about || '',
        idProof: processedData.idProof || undefined,
        policeVerification: processedData.policeVerification || undefined,
        rentPayments: processedData.rentPayments || [],
        electricityRecords: processedData.electricityRecords || [],
        active: false,
        moveInDate: processedData.moveInDate || null,
        moveOutDate: processedData.moveOutDate || new Date(),
        ownerId: processedData.ownerId || currentUser.uid, // Ensure ownerId is set
      };
      
      console.log("Processed previous tenant:", tenant.name);
      return tenant;
    });
    
    console.log("Fetched previous tenants count:", tenants.length);
    return tenants;
  } catch (error) {
    console.error("Error fetching previous tenants:", error);
    throw error;
  }
};
...
export const subscribeToTenants = (callback: (tenants: Tenant[]) => void) => {
  try {
    const currentUser = auth.currentUser;
    if (!currentUser) {
      console.warn("No authenticated user for tenants subscription");
      callback([]);
      return () => {};
    }
    
    console.log("Setting up previous tenants subscription for user:", currentUser.uid);
    const q = query(previousTenantsCol, where("ownerId", "==", currentUser.uid));
    
    return onSnapshot(q, snapshot => {
      if (snapshot.empty) {
        console.log("No previous tenants documents found in subscription");
        callback([]);
        return;
      }
      ...
      const tenants = snapshot.docs.map(doc => {
        const rawData = doc.data();
        console.log("Raw tenant data from subscription:", { id: doc.id, ...rawData });
        ...
        console.log("Processed tenant from subscription:", tenant.name);
        return tenant;
      });
      
      console.log("Previous tenants subscription updated:", tenants.length, "tenants");
      callback(tenants);
    }, error => {
      console.error("Error subscribing to tenants:", error);
    });
  } catch (error) {
    console.error("Error setting up tenants subscription:", error);
    return () => {}; // Return empty function in case of error
  }
};

export const addTenantToPrevious = async (tenant: Tenant): Promise<string> => {
  try {
    const currentUser = auth.currentUser;
    if (!currentUser) {
      console.error("User not authenticated when adding tenant to previous");
      throw new Error("User not authenticated");
    }
    
    console.log("Adding tenant to previous:", tenant.name);
    
    // Make sure the tenant object doesn't have any undefined values before adding to Firestore
    const cleanTenant = Object.entries(tenant).reduce((acc, [key, value]) => {
      if (value !== undefined) {
        acc[key] = value;
      }
      return acc;
    ...
    const tenantToAdd = {
      ...cleanTenant,
      ownerId: currentUser.uid, // Ensure owner ID is set
      name: tenant.name, // Explicitly include name
      contactNo: tenant.contactNo, // Explicitly include contactNo
      memberCount: tenant.memberCount, // Explicitly include memberCount
      rentAmount: tenant.rentAmount, // Explicitly include rentAmount
      roomDetails: tenant.roomDetails, // Explicitly include roomDetails
      about: tenant.about, // Explicitly include about
      moveInDate: tenant.moveInDate || null,
      moveOutDate: tenant.moveOutDate || new Date(),
      active: false
    };
    
    console.log("Prepared tenant data for adding to previous:", {
      name: tenantToAdd.name,
      ownerId: tenantToAdd.ownerId,
      moveInDate: tenantToAdd.moveInDate,
      moveOutDate: tenantToAdd.moveOutDate
    });
    
    const docRef = await addDoc(previousTenantsCol, tenantToAdd);
    console.log("Tenant added to previous with ID:", docRef.id);
    return docRef.id;
  } catch (error) {
    console.error("Error adding tenant to previous:", error);
    throw error;
  }
};
